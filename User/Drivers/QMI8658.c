...
// for ChiTu
#define I2C_Write_Reg(addr, reg, buf, len) (IIC_WriteLen(addr, reg, len, buf))
#define I2C_Read_Reg(addr, reg, buf, len) (IIC_ReadLen(addr, reg, len, buf))
...
#else
		ret = I2C_Write_Reg(g_imu.slave, reg, &value, 1);
#endif
	}
	return ret;
}
...
#else
		ret = I2C_Read_Reg(g_imu.slave, reg, buf, len);
#endif
	}
	return ret;
}
...
void qmi8658_data_cali(unsigned char sensor, float data[3])
{
	...
	} else if (sensor == 2) // gyroscope
	{
		...
		if (((data_diff[0] + data_diff[1] + data_diff[2]) < 0.03f) && ((data[0] > -1.0f) && (data[0] < 1.0f)) &&
		    ((data[1] > -1.0f) && (data[1] < 1.0f)) && ((data[2] > -1.0f) && (data[2] < 1.0f))) {
...
void qmi8658_config_acc(enum qmi8658_AccRange range, enum qmi8658_AccOdr odr, enum qmi8658_LpfConfig lpfEnable,
...
void qmi8658_config_gyro(enum qmi8658_GyrRange range, enum qmi8658_GyrOdr odr, enum qmi8658_LpfConfig lpfEnable,
			 enum qmi8658_StConfig stEnable)
{
...
void qmi8658_enable_no_motion(unsigned char enable, enum qmi8658_Interrupt int_map)
{
	if (enable) {
		...
		qmi8658_config_gyro(Qmi8658GyrRange_1024dps, Qmi8658GyrOdr_62_5Hz, Qmi8658Lpf_Disable,
				    Qmi8658St_Disable);
...
void qmi8658_enable_sig_motion(unsigned char enable, enum qmi8658_Interrupt int_map)
{
	if (enable) {
		...
		g_imu.cfg.ctrl8_value |=
			QMI8658_CTRL8_SIGMOTION_EN | QMI8658_CTRL8_ANYMOTION_EN | QMI8658_CTRL8_NOMOTION_EN;
		qmi8658_write_reg(Qmi8658Register_Ctrl8, g_imu.cfg.ctrl8_value);
		qmi8658_delay(1);
...
void qmi8658_config_fifo(unsigned char watermark, enum qmi8658_FifoSize size, enum qmi8658_FifoMode mode,
			 enum qmi8658_Interrupt int_map, uint8_t sensor)
{
	unsigned char ctrl1;
