...
u8 AP3216C_Init(void)
{
	...
	IIC_Init(100000, 0x02); // 建议先用 100kHz 低速测试稳定性
	...
	// 2. 配置工作模式
	AP3216C_WriteOneByte(0x00, 0X03);
	Delay_Ms(10); // 等待配置生效

	// 3. 读取状态寄存器 (通常读取 0x00 或特定的 ID 寄存器验证)
	temp = AP3216C_ReadOneByte(0X00);

	// 4. 判断
	// 注意：有些传感器读回来的值可能包含状态位，不一定完全等于写入值
	// 只要低2位是 11 (即 0x03)，通常就算成功
	if ((temp & 0x03) == 0x03)
		return 0; // 成功
	else
		return 1; // 失败
